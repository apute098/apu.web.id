...
2. **AI Extractor (Gemini 3.6 Flash)**: Mengolah pesan transaksi berbahasa manusia menjadi JSON transaksi terstruktur.
...
3. **Ekstraksi AI (Gemini 3.6 Flash)**:
...
- `POST /api/v1/system-status` dengan `{ "action": "toggle_stress" }` -> Simulasi beban CPU.
...
### D. Endpoint Export Bundle:
- `GET /api/v1/export-json` -> Menghasilkan `project.json` untuk deployment 1-line extractor Arch Linux.
