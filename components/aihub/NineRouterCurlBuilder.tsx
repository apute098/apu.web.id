...
export const NineRouterCurlBuilder: React.FC = () => {
  ...
  return (
    <div className="p-1 rounded-none bg-slate-900 border border-slate-700 border border-white/10 shadow-xl ">
      <div className="rounded-none bg-slate-950 p-5 space-y-4">
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            <Sliders className="w-4 h-4 text-cyan-400" />
            <h4 className="text-sm font-bold text-slate-200">Interactive cURL Generator</h4>
          </div>
          <span className="text-[11px] font-mono text-slate-400 bg-slate-900 border border-slate-700 px-2 py-0.5 rounded-none border border-white/10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full rounded-none bg-slate-900/80 border border-white/10 px-3 py-2 text-slate-200 focus:outline-none focus:border-cyan-500/50"
            >
        ...
        <div className="relative rounded-none bg-slate-950/90 border border-white/10 p-4 font-mono text-[11px] text-cyan-300/90 overflow-x-auto">
          <pre className="whitespace-pre">{curlCommand}</pre>
          <button
            onClick={handleCopy}
            className="absolute top-3 right-3 flex items-center gap-1.5 px-3 py-1.5 rounded-none bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 hover:bg-cyan-500/30 active:scale-95 transition-all text-xs font-semibold"
          >
            {copied ? (
              <>
